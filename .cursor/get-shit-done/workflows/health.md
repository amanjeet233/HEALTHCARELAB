...
```bash
node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.cursor/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.cursor/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.cursor/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.cursor/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.cursor/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
