...
2. Config value: `node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.windsurf/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.windsurf/get-shit-done/bin/gsd-tools.cjs" config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  if [ "$HAS_STATUS" = "valid" ]; then
    ...
    if [ "$COMMIT_DOCS" = "true" ]; then
      node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.windsurf/get-shit-done/bin/gsd-tools.cjs" commit \
