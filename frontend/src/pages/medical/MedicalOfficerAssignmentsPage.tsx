...
import { RefreshCw, UserCheck, CalendarClock, AlertCircle } from 'lucide-react';
...
const MedicalOfficerAssignmentsPage: React.FC = () => {
  ...
  const uniqueDates = useMemo(
    () => [...new Set(bookings.map((b) => b.bookingDate).filter(Boolean))] as string[],
    [bookings]
  );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const rows = (resp.data?.data || []) as UnassignedBooking[];
      setBookings(rows);
    } catch {
  ...
  useEffect(() => {
    void Promise.all(uniqueDates.map((date) => loadTechsByDate(date)));
  ...
  const handleAssign = async (booking: UnassignedBooking) => {
  ...
  const handleOpenBookingDetails = (booking: UnassignedBooking) => {
  ...
  return (
    <div className="max-w-[1140px] mx-auto px-5 md:px-6 lg:px-8 py-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-[#164E63]">Staff <span className="text-cyan-600">Tasks</span></h1>
          <p className="text-sm text-slate-500 font-semibold">Assign staff to new bookings and check their work.</p>
        </div>
        <button
          onClick={loadData}
      ...
      {loading ? (
        ...
        <MOEmptyState
          title="No new tasks found."
          description="No new bookings matched current filters."
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="space-y-2 min-w-0 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[10px] font-black px-2 py-1 rounded-md bg-amber-100 text-amber-700">UNASSIGNED</span>
                    </div>
                    <p className="text-xs text-slate-600 font-semibold">
                      {booking.patientName || 'Patient'} • {booking.bookingDate || '-'} • {booking.timeSlot || '-'}
                    </p>
                    {booking.collectionAddress && (
                      <p className="text-xs text-slate-500">Address: {booking.collectionAddress}</p>
                    )}
                    <MOStatusTimeline
                  ...
                  <div className="w-full md:w-[340px]" onClick={(e) => e.stopPropagation()}>
                    ...
                    {options.length === 0 && !loadingTechByDate[date] && (
                      <div className="mt-2 inline-flex items-start gap-1.5 text-[11px] text-amber-700 font-semibold">
                        <AlertCircle className="w-3.5 h-3.5 mt-[1px]" />
                        No active technician data available for {date || 'this date'}.
                      </div>
                    )}
                    ...
                    <button
                      ...
                      className="mt-3 inline-flex items-center justify-center gap-1.5 w-full px-4 py-2 bg-teal-600 text-white rounded-lg text-xs font-black uppercase disabled:opacity-50"
                    >
                      <UserCheck className="w-3.5 h-3.5" /> {busy ? 'Assigning...' : 'Assign Technician'}
                    </button>
                  </div>
                </div>
              </div>
...
export default MedicalOfficerAssignmentsPage;
