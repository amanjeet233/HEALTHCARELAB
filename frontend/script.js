const fs = require('fs');
const file = 'src/pages/TestListingPage.tsx';
let txt = fs.readFileSync(file, 'utf8');
const cardStart = txt.indexOf('<Card');
const cardEndMatch = '                            ))}';
const cardEnd = txt.indexOf(cardEndMatch, cardStart);

if (cardStart > -1 && cardEnd > -1) {
    const cardContent = \                                <Card
                                    key={test.id}
                                    className="group overflow-hidden border border-gray-100 hover:border-[#0D7C7C]/30 hover:shadow-lg transition-all duration-300 cursor-pointer bg-white rounded-2xl"
                                    noPadding
...
\;
    txt = txt.substring(0, cardStart) + cardContent + txt.substring(cardEnd);
    fs.writeFileSync(file, txt);
    console.log("Updated Cards!");
} else {
    console.log("Could not find card bounds.");
}
