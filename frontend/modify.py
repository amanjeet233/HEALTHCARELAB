...
start_idx = txt.find('<Card')
end_idx = txt.find('</Card>', start_idx) + 7

if start_idx != -1 and end_idx != -1:
    new_card = '''<Card key={test.id} className="group overflow-hidden border border-gray-100 hover:border-[#0D7C7C]/30 hover:shadow-lg transition-all duration-300 cursor-pointer bg-white rounded-2xl" noPadding onClick={() => navigate(/booking/)}>
    ...
    txt = txt[:start_idx] + new_card + txt[end_idx:]
    with open('src/pages/TestListingPage.tsx', 'w', encoding='utf-8') as f:
        f.write(txt)
    print("Listing cards updated.")
else:
    print("Failed to find Card bounds.")
