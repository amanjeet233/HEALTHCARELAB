...
- **Global scope**: `D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.claude/notes/`)
...
2. Glob `D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.claude/notes/` — works in any directory
