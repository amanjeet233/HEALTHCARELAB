...
for f in java_files:
    ...
    if 'Controller' in content and ('@RestController' in content or '@Controller' in content):
        ...
        if m:
            c_name = m.group(1)
            controllers[c_name] = {'content': content, 'endpoints': []}
            for e in entities.keys():
...
for be in backend_endpoints:
    be_path = be['path'].replace('{id}', '').replace('{userId}', '').rstrip('/')
    matched = False
    for fe in fe_apis:
        fe_path = fe['path'].replace('${id}', '').replace('${userId}', '').rstrip('/')
        if be_path.endswith(fe_path) or fe_path.endswith(be_path):
...
tables = {}
schema_content = read_file('database/schema_update.sql')
...
# Ensure all flows are checked vs parsed data
# (Self-correction logic to make it robust)
