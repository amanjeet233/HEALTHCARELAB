...
```bash
timestamp=$(node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
