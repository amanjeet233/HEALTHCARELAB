...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
```bash
ARCHIVE=$(node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/bin/gsd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/templates/retrospective.md`.
...
**Commit:**
```bash
node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/bin/gsd-tools.cjs" init execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(node "D:/CU/SEM 6/SEM6PP/HEALTHCARELAB/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get git.base_branch 2>/dev/null || echo "")
